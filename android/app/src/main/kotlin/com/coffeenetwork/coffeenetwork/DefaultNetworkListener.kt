// Adapted from sing-box-for-android (GPLv3) — Mygod Studio / SagerNet.
...
object DefaultNetworkListener {
        ...
        for (message in channel) {
            when (message) {
                ...
                is NetworkMessage.Update ->
                    if (network == message.network) listeners.values.forEach { it(network) }
    ...
    suspend fun get(): Network = if (fallback) {
        @TargetApi(23)
        App.connectivity.activeNetwork ?: error("missing default network")
    } else {
